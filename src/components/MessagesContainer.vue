...
defineProps({
  messages: {
    type: Array as PropType<ListMessage[]>,
    required: true
  }
})
...
const handleGetMessage = (message_index: number, message: any) => {
  ...
  if (content_messages.value[message.link]) return
  ...
  <div class="absolute h-screen top-0 w-full bg-[var(--tg-theme-bg-color)]">
    <div class="w-full">
      <button @click="emit('close')" class="p-2">
        <svg
          class="w-6 h-6 text-[var(--tg-theme-hint-color)]"
          aria-hidden="true"
    ...
    <div id="accordion-collapse" data-accordion="collapse">
      <template v-for="(message, key) in messages" :key="key">
        <h2 :id="`accordion-collapse-heading-${key}`">
          <button
            @click="handleGetMessage(key, message)"
            type="button"
        ...
        >
          ...
          <div
            v-else
            class="p-5 bg-[var(--tg-theme-bg-color)] border border-b-0 border-gray-700 text-[var(--tg-theme-hint-color)] overflow-auto"
          >
            <div v-html="content_messages[message.link]?.content"></div>
          </div>
        </div>
      </template>
    </div>
